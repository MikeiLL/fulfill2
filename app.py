...
def deleteproducer():
    ...
    with _conn, _conn.cursor() as cur:
        cur.execute("DELETE FROM producers WHERE id = %s", (data['id'],))
    with _conn, _conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute("SELECT id, company, email, phone FROM producers ORDER BY company")
        state['producers'] = cur.fetchall()
    update_sockets("producers")
    return jsonify(state)

@app.route('/newproducer', methods=['POST'])
def newproducer():
    if (current_user.user_level != 3):
        return "Unauthorized", 401
    data = request.json
    producer = json.loads(data['form'])
    #if not json['name'] in ['company','email', 'phone']: return
    # for now ignore other values
    # TODO maybe sanitize https://pathvalidate.readthedocs.io is one way
    #we don't want to create multiple producers for each field
    #probably want to create the producer no matter which field received
    ...
    try:
        with _conn, _conn.cursor() as cur:
            cur.execute("""INSERT INTO producers (company, email, phone, web)
            VALUES (%s, %s, %s, %s) RETURNING id""", (producer['company'], producer['email'], producer['phone'], producer['web']))
            state['item_id'] = cur.fetchone()[0]
    except (psycopg2.errors.CheckViolation, psycopg2.errors.UniqueViolation) as e:
        state["error"] = str(e)
        message = str(e)
        if "at_least_one_producer_point" in message:
            message = "Enter at least one producer contact method."
        elif "unique constraint"  in message:
            message = "You may be trying to enter an email or phone that's already present."
        update_sockets("producers", {"error": message, "cmd": "update"})
        return jsonify(state)
    with _conn, _conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
...
def newproduct():
    if (current_user.user_level != 3):
        return "Unauthorized", 401
    data = request.json
    product = json.loads(data['form'])
    print("product name", product['name'], type(product['name']))
    #if not json['name'] in ['company','email', 'phone']: return
    # for now ignore other values
    # TODO maybe sanitize https://pathvalidate.readthedocs.io is one way
    #we don't want to create multiple producers for each field
    ...
    #try:
    with _conn, _conn.cursor() as cur:
        cur.execute("""INSERT INTO products (name, producer_id)
        VALUES (%s, %s) RETURNING id""", (product['name'], data['group'].split("-")[1]))
        state['item_id'] = cur.fetchone()[0]
    """ except Exception as e:
        state["error"] = str(e)
        message = str(e)
        update_sockets(data['group'], {"error": message, "cmd": "update"})
        return jsonify(state) """
    with _conn, _conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        cur.execute("SELECT id, name FROM products ORDER BY name")
        state['products'] = cur.fetchall()
